...
import { D10Section as D10SectionType, DivisionalDomainMetadata, Planet } from '../../types';
...
interface D10SectionProps {
  readonly section: D10SectionType;
}

export const D10Section: React.FC<D10SectionProps> = ({ section }) => {
  if (section.status === 'UNAVAILABLE' || !section.details) {
    return <EmptyState title="Dashamsha (D10) Analysis Unavailable" message="D10 divisional interpretation was not provided in the report." />;
  }
  ...
  return (
    <div className="space-y-4">
      {section.status === 'PARTIAL' && <PartialStateNotice message="Dashamsha (D10) data is partial." />}

      {/* Main Header Info */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800">
          <span className="text-[10px] font-mono-code uppercase text-slate-400 block">D10 Ascendant Sign</span>
      ...
      {details.domainMetadata && Object.keys(details.domainMetadata).length > 0 && (
        <div className="bg-slate-950/70 border border-slate-800 rounded-xl p-4 space-y-2">
          <h4 className="text-xs font-mono-code font-bold uppercase text-slate-300">
            Domain Metadata & Career / Purpose Focus
          </h4>
      ...
      {details.evidence && details.evidence.length > 0 && (
        <EvidenceList evidence={details.evidence} title="D10 Dashamsha Interpretation Evidence" />
      )}
    </div>
  );
};
