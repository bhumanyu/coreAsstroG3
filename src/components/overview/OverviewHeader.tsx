...
import type { OverviewChart } from '../../product/analysis/overviewViewModel';
import type { ProductWarning } from '../../product/analysis/productAnalysisTypes';
import { Compass, Moon, Sun, Star, AlertTriangle } from 'lucide-react';
...
export const OverviewHeader: React.FC<OverviewHeaderProps> = ({
  ...
  const ascendant = chart?.ascendantSign || 'Unavailable';
  const moon = chart?.moonSign || 'Unavailable';
  const sun = chart?.sunSign || 'Unavailable';
  const nakshatra = chart?.moonNakshatra || 'Unavailable';

  return (
    <div className="space-y-4">
      <PageHeading
        eyebrow="Overview & Key Findings"
        title="Unified Life Domain Analysis"
        description="Holistic astrological synthesis across career vocation, wealth potential, and active planetary timing."
      >
        {warnings.length > 0 && (
