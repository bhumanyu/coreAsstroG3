/**
 * Career Presentation Formatters (P-UI-04)
 *
...
/**
 * Format D10 relationship values:
 * 'CONFIRMS' | 'PARTIALLY_CONFIRMS' | 'MODIFIES' | 'CONFLICTS' | 'UNAVAILABLE'
 */
export function formatD10Relationship(rel?: string): string {
...
/**
 * Tailwind styling helper for D10 Relationship badges
 */
export function getD10RelationshipBadgeClass(rel?: string): string {
  switch (rel) {
    case 'CONFIRMS':
      return 'bg-emerald-950/60 border-emerald-800/80 text-emerald-300';
    case 'PARTIALLY_CONFIRMS':
