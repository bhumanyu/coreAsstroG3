...
export function getD10EvaluationDiagnostics(
  ...
  const getDignityFromFlat = (planetInfo: any): string | undefined => {
    if (!planetInfo) return undefined;
    if (typeof planetInfo.dignity === 'string') return planetInfo.dignity;
    if (planetInfo.dignity?.status) return planetInfo.dignity.status;
    return undefined;
  };

  const d10LordDignity = getDignityFromFlat(d10LordPlanetInfo);
  const natal10LDignity = getDignityFromFlat(natal10LInD10);

  const isD10LordStrong =
    d10LordDignity === 'EXALTED' ||
    d10LordDignity === 'OWN_SIGN' ||
    d10LordPlanetInfo?.house === 10;

  const isNatal10LStrongInD10 =
    natal10LDignity === 'EXALTED' ||
    natal10LDignity === 'OWN_SIGN' ||
    isD10Vargottama ||
    natal10LInD10?.house === 10;

  const isD10LordAdverse = d10LordDignity === 'DEBILITATED';
  const isNatal10LAdverseInD10 = natal10LDignity === 'DEBILITATED';

  const d10SupportFactors: string[] = [];
  if (d10LordDignity === 'EXALTED') d10SupportFactors.push(`D10 10th lord (${d10Lord}) is exalted in D10`);
  if (d10LordDignity === 'OWN_SIGN') d10SupportFactors.push(`D10 10th lord (${d10Lord}) is in own sign in D10`);
  if (d10LordPlanetInfo?.house === 10) d10SupportFactors.push(`D10 10th lord (${d10Lord}) is placed in 10th house of D10`);
  if (natal10LDignity === 'EXALTED') d10SupportFactors.push(`Natal 10th lord (${natal10L}) is exalted in D10`);
  if (natal10LDignity === 'OWN_SIGN') d10SupportFactors.push(`Natal 10th lord (${natal10L}) is in own sign in D10`);
  ...
  return Object.freeze({
    ...
    d10LordDignity: d10LordDignity,
    natal10Lord: natal10L,
    natal10LordHouse: natal10LInD10?.house,
    natal10LordDignity: natal10LDignity,
